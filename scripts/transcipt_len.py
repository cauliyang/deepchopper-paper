...
def get_transcript_lengths(
    ...
    if show_stats:
        ...
        if 'gene_biotype' in gene_info.columns:
            biotype_counts = gene_info.group_by('gene_biotype').agg(
    ...
    transcript_lengths = exons.with_columns(
        (pl.col('end') - pl.col('start') + 1).alias('exon_length')
    ).group_by('transcript_id').agg(agg_cols).with_columns(
    ...
    if output_file:
        ...
        for row in result.iter_rows(named=True):
            if max_only:
                ...
                print(f"{row['transcript_id']}\t{row['gene_id']}\t{row['transcript_length']}")
...
def main(
    ...
    gene_biotype: str = typer.Option("protein_coding", "--biotype", "-b", help="Filter by gene biotype (e.g., 'protein_coding')"),
    show_stats: bool = typer.Option(False, "--stats", "-s", help="Show gene biotype statistics"),
    max_only: bool = typer.Option(False, "--max-only", "-m", help="Output only maximum transcript length per gene (default: all transcripts)")
):
    """
    Calculate transcript lengths and genomic spans from a GTF file.
