...
\begin{tikzpicture}[
	...
	connect/.style={rounded corners, black!30, text=black, thick, -{latex[width=0.5mm]}, shorten >=1pt, shorten <=1pt},
    ...
    \begin{scope}[shift={(exon.center)}, yshift=-1.5cm, xshift=9,  start chain=1 going below, node distance=0mm, scale=0.8, transform shape]
		\matrix (rnaseq) [matrix of nodes, nodes in empty cells,
			% column sep=-\pgflinewidth, row sep=-\pgflinewidth,
			column sep=-\pgflinewidth, row sep=-\pgflinewidth,
			nodes={thin, inner sep=0pt, outer sep=0pt, minimum width=1em, minimum height=1em},
			row 1 column 1/.style={nodes={fill=p1}},
			row 1 column 2/.style={nodes={fill=p1}},
			row 1 column 3/.style={nodes={fill=p3}},
			row 1 column 4/.style={nodes={fill=p3}},
			row 1 column 5/.style={nodes={fill=p1}},
			row 1 column 6/.style={nodes={fill=p1}},
			row 1 column 7/.style={nodes={fill=p1}},
		] {
			 0 & 0 & 1 & 1 & 0 & 0 & 0 \\
			 A & T & T & T & C & G & G \\
		};
        \node (rnaseq-label) [left=1pt of rnaseq-2-1] {RNA};
        \node (label) [left=1pt of rnaseq-1-1] {Label};
        \node (databox) [fit=(rnaseq)(rnaseq-label)(label), draw=c7, dashed, thick, inner sep=0] {};
	\end{scope}

	\begin{scope}[local bounding box=tokens, shift={(databox.south)}, yshift=-1cm]
		\matrix (tokens1)   [
			matrix of nodes,
	...
	\node (lcglm) [below of=tokens1, yshift=-0.4cm, block, fill=l1,  thick, minimum height = 3em] {Long-Context GLM};

	\begin{scope}[shift={(lcglm.south)}, yshift=-0.8cm, xshift=3, local bounding box=features]
	...
	\node (mlp) [below of=features2, yshift=-0.3cm, rectangle, text centered, rounded corners, minimum width=5em, minimum height=2em,  fill=c6] {MLP};

	\begin{scope}[local bounding box=outputbox, shift={(mlp.south)}, yshift=-0.8cm]
		\matrix (output)   [
			matrix of nodes,
			nodes in empty cells,
			nodes={fill=c2, draw=black, thin, outer sep=0pt},
    ...
    \begin{scope}[shift={(output)}, yshift=-1cm, local bounding box=prediction]
			...
			% row 1 column 2/.style={nodes={fill=p1}},
			% row 1 column 3/.style={nodes={fill=p3}},
			row 1 column 3/.style={nodes={fill=p3, text=white}},
    ...
    (databox) edge["Tokenizer", connect, font=\tiny \tt] (tokens)
	(tokens) edge[connect] (lcglm)
	(lcglm) edge[connect] (features)
	(features) edge[connect] (mlp)
	(mlp) edge[connect] (output)
	(output) edge["Predict", connect, font=\tiny \tt] (prediction)
	edge[blue,  skip loop={18mm}{Loss}] (rnaseq-1-7);
\end{tikzpicture}
