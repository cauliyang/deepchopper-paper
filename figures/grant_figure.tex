...
\begin{tikzpicture}[
	...
	% Exitron and Exon
	\begin{scope}[local bounding box=exon, scale=0.7, transform shape]
		...
		\node (left) [left= 16pt of exon-n1]  {};
		\node (right) [right= 16pt of exon-n3]   {};

		\path (left) edge[-, thick]  (exon-n1)
		(exon-n3) edge[-, thick] (right);
	\end{scope}

    \node (dnalabel) [above=8pt of exon-n1, xshift=-11pt] {\tiny \ttfamily Ultra-long DNA};

	% RNAseq Matrix
	\begin{scope}[shift={(exon.center)},
			yshift=-1.1cm, xshift=8,
	...
	\end{scope}
		...
		\node[inner sep=3pt, anchor=north west] at (lcglmbox.north west) {\tiny \ttfamily Long-Context GLM};
        \end{pgfonlayer}
    ...
    \node [left = of output, xshift=0.7cm, anchor=center] {\tiny \ttfamily Output};
