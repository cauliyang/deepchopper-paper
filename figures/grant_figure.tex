...
% We propose a deep learning model for analyzing ultra-long DNA sequences using a Long-Context Genomic Language Model (GLM) to predict exitron existence from DNA sequences. The methodology is outlined in several key steps (Figure 1A).

% The input consists of ultra-long sequences, which may include exons and exitrons, with specific nucleotide patterns marked in Figure 1A. During the training phase, sequences are encoded with labels, where exitron sequences are labeled as 1 and other sequences as 0. Sequencing data is processed by a tokenizer, which converts the sequence data into tokens. These tokens are represented by a series of colored blocks in Figure 1A. The tokenized data is then input into a long-context GLM, a model designed to handle long-range dependencies and consider distant relationships within the sequence (Figure 1A).
