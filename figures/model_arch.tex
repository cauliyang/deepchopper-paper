...
\begin{tikzpicture}[
	font=\ttfamily,
	block/.style = {rectangle, draw, fill=c2, text centered, rounded corners, minimum height=4em, minimum width=4em},
	connect/.style={rounded corners, black!50, text=black, ultra thick, -{Stealth[length=5pt, width=4pt]}, shorten >=1pt, shorten <=1pt},
	]
	\def\boxdist{2cm}

	% input
	\begin{scope}[ yshift=0cm, local bounding box=inputbox]
		\node (input) [rotate=270] {
		...
		\begin{scope}[on background layer]
			\node (intputoutline) [rotate=270, fit=(rnaseq), draw=mygreen,  thick, fill=mygreen!10, rounded corners, inner sep=1pt] {};
		\end{scope}
	\end{scope}
	...
	\begin{scope}[shift={(featuresbox.south)}, yshift=-\boxdist, local bounding box=qualblockbox]
		\node [block] (qualblock) {Qual Block};
	\end{scope}
	...
	% mlp
	\begin{scope}[shift={(qualbox.south)}, yshift=-\boxdist, local bounding box=mlpbox]
		\node [block] (mlp) {MLP};
	\end{scope}
	...
	\path
	(inputbox)  edge[connect] (tokenbox)
	(tokenbox) edge[connect] (lcgmlbox)
	(lcgmlbox) edge[connect] (featuresbox)
	(featuresbox) edge[connect] (qualblockbox)
	(qualbox) edge[connect] (qualblockbox)
	(mlpbox) edge[connect] (outputbox)
	(outputbox) edge[connect] (predictbox)
	;

	\draw [connect] (qualblockbox) |- (mlp);
\end{tikzpicture}
