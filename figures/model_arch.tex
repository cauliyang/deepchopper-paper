...
\begin{tikzpicture}[
	...
	connect/.style={rounded corners, black!60, text=black, very thick, -{Stealth[length=5pt, width=4pt]}, shorten >=1pt, shorten <=1pt},
	input box/.style={fill=mygreen!20,  thick, rounded corners, inner sep=5pt},
	output box/.style={fill=c7!20,  thick, rounded corners, inner sep=5pt},
	token node/.style={ draw=black, line width=0.5pt, minimum size=1.2em},
	]
	\def\boxdist{1cm}

	\begin{scope}[local bounding box=nanoporebox]
		\node (nanoporenode) [block, rotate=270] {Nanopore};
	\end{scope}

	% input
	\begin{scope}[shift={(nanoporebox.east)}, xshift=\boxdist, local bounding box=inputbox]
		\node (rnaseqnode) [
			rotate=270,
		] {
			\tikz \matrix (rnaseq) [matrix of nodes, nodes in empty cells,
				...
				row 3/.style={nodes={fill=p1, draw=p1}},
				row 3 column 4/.style={nodes={fill=p3, draw=p3}},
				...
				T & $\cdot$ & C & A & A & T & G & T & $\cdot$ & G \\
				T & $\cdot$ & C & A & A & T & G & T & $\cdot$ & G \\
				0 & $\cdot$ & 0 & 1 & 1 & 1 & 1 & 0 & $\cdot$ & 0 \\
			};
		};
	...
	\begin{scope}[shift={(tokenbox.east)}, xshift=\boxdist, yshift=-0.4cm,
		edge1/.style={black, line width=0.8pt, -{Stealth[length=3pt, width=3pt]}, shorten >=0.5pt, shorten <=0.5pt},
		...
		\node (lcgmlboxoutline) [fit=(lcgmlbox), fill=l1, draw, rounded corners,  thick, inner sep=6pt, inner ysep=12pt] {};
		...
		\begin{pgfonlayer}{middle2}
			...
			node (lcgmlboxoutline2) [draw, fill=l1, thick,rounded corners, minimum width=\x2-\x1, minimum height=\y2-\y1, opacity=0.9]
	...
	% qual block
	\begin{scope}[shift={(featuresbox.east)}, xshift=\boxdist  * 1.5, yshift=-0.1cm, local bounding box=qualblockbox,
        scale=1.1,
        transform shape,
        qual block/.style = {rectangle, fill=q4, text centered, rounded corners, minimum height=1em, minimum width=4em},
        resuidual block/.style = {rectangle, fill=q5, text centered, rounded corners, minimum height=4em, minimum width=4em},
        seq block/.style = {rectangle, fill=q6, text centered, rounded corners, minimum height=1em, minimum width=4em},
        ]

        \node [seq block, rotate=270] (sblock) {seq};
        \node [resuidual block, right=8pt of sblock, yshift=20pt] (rblock) {residual};
		\node [qual block, below=1pt of rblock] (qblock) {qual};
	...
	% mlp
	\begin{scope}[shift={(qualblockbox.east)}, xshift=\boxdist  * 0.1, yshift=0.4cm,
	...
	%  smoutput
	\begin{scope}[shift={(outputbox.east)}, xshift=\boxdist * 1.5, local bounding box=smoutputbox]
	...
	(inputbox)  edge ["Tokenization", font=\tiny] (tokenbox)
	(tokenbox) edge (lcgmlbox)
	(lcgmlbox) edge (featuresbox)
	(featuresbox) edge (qualblockbox)
	(mlpbox) edge (outputbox)
	(outputbox) edge["Polish", above, font=\tiny] (smoutputbox)
	(smoutputbox) edge["Predict", above, font=\tiny] (predictbox)
	(qualblockbox) edge (mlpbox)
	;


	\begin{scope}[on background layer]
		\node (intputoutline) [fit=(rnaseqnode)(nanoporenode), input box] {};
		\node (modeloutline) [fit=(tokenbox)(lcgmlbox)(qualblockbox)(mlpbox)(outputbox), rectangle, inner sep=10pt, inner ysep=3pt, fill=c1] {};
		\node[inner sep=1pt, anchor=north west] at (modeloutline.north west) {Model};
		\node (outputoutline) [fit=(predictbox)(smoutputbox), output box] {};

        \node (mlplabel) [above=4pt of mlpbox] {MLP};
        \node (inputlabel) [above=4pt of intputoutline] {Input};
        \node (outputlabel) [above=4pt of outputoutline] {Output};
	\end{scope}

    \draw [connect]  ($(intputoutline.south) + (24pt, 0)$) -- ++(0, -18pt)  -| ($(qblock) + (0, -14pt)$);
