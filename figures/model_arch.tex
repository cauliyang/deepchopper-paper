...
\begin{tikzpicture}[
		...
		connect/.style={-stealth, ultra thick, shorten >=1},
	]
	...
	\begin{scope}[shift={($(seqbox.south)+(-2,-8)$)}, scale=1, local bounding box=modelbox1]
		...
		% add qual unber add1
		\node[below= \boxdist of add1] (qual) {\tt 5>SS\dots S\textcolor{mymauve}{,*\%\%\%\#\$,}SS\dots <HL};
		...
		\draw[connect] (output) -- (final_feature) node[above] {};

		% add arrow from qual to add1
		\draw[connect] ($(qual) + (0,1)$) -- (add1);
