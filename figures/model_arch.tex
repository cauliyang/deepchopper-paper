...
\begin{tikzpicture}[
	...
	connect/.style={rounded corners, black!40, text=black, ultra thick, -{Stealth[length=5pt, width=4pt]}, shorten >=1pt, shorten <=1pt},
	input box/.style={draw=mygreen, dashed,  thick, rounded corners, inner sep=-2pt},
	output box/.style={draw=q6,     dashed, thick, rounded corners, inner sep=-2pt},
	]
	\def\boxdist{1cm}
	...
	\begin{scope}[ yshift=1cm, local bounding box=inputbox]
		...
		\begin{scope}[on background layer]
			\node (intputoutline) [fit=(inputnode), input box] {};
		\end{scope}

	\end{scope}

	% token
	\begin{scope}[shift={(inputbox.east)}, xshift=\boxdist * 1.5, local bounding box=tokenbox]
		\matrix (tokens1)   [
			matrix of nodes,
	...
	% lcglm
	\begin{scope}[shift={(tokenbox.east)}, xshift=\boxdist * 2, local bounding box=lcgmlbox]
	...
	\begin{scope}[shift={(qualbox.west)}, xshift=-\boxdist, local bounding box=qualseqbox]
		...
		\begin{scope}[on background layer]
			\node (qualseqoutline) [fit=(qualseqnode),input box] {};
		\end{scope}
	\end{scope}
	...
	\begin{scope}[shift={(outputbox.west)}, xshift=-\boxdist, local bounding box=predictbox]
		...
		\node (predictoutline) [fit=(predictnode), output box] {};
	\end{scope}
	...
	(outputmatrix-1-1)  edge["Loss", sloped, font=\tiny \ttfamily] (rnaseq-2-8)
	;

	\draw [connect] (outputbox) -- node[above,font=\tiny \ttfamily]{Predict} (predictbox) ;
