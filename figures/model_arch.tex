...
\begin{tikzpicture}[
		node/.style={thick,circle,draw=myblue,minimum size=1.5em, inner sep=0.5,outer sep=0.6},
		node hidden/.style={node,
				very thick,
				draw=myblue,
				% blue!20!black,
		...
		node embed/.style={very thick, circl, minimum size=0.5em, fill=mygreen!20,draw=mygreen!30!black},
		connect/.style={-stealth, ultra thick, shorten <=2pt, shorten >=2pt},
		% make all text bigger/.style={font=\Large},
	...
	\begin{scope}[shift={($(seqbox.south)+(-2,-8)$)}, scale=1, local bounding box=modelbox1]
		...
		\draw[connect] ([xshift=20em]seq.west) -- node[midway, above] {Tokenization} (token);
		% add arrow from token to embedding
		\draw[connect] ([xshift=8em]token.west) -- node[midway, above] {Feature Extract} (embed);
		...
		\node[\boxcolor, right = 1.5 * \boxdist of embed, draw, very thick, rounded corners, minimum size=\boxsize * 1.25] (weight_layer1) {};
		...
		\node[right= \boxdist of weight_layer1, font=\Huge, minimum size=0.5 * \boxsize] (add1) {$\oplus$};

		% add weight layer 2
		\node[\boxcolor, right= \boxdist of add1, rectangle, draw, very thick, rounded corners, minimum size=\boxsize * 1.25] (weight_layer2) {};
		...
		\node[\boxcolor, rectangle, draw, very thick, rounded corners, minimum size=\boxsize, right = \boxdist of add2] (output) {};
		...
		% let the label parallel to the line
