...
\begin{tikzpicture}[
	...
	\begin{scope}[shift={($(seqbox.south)+(-2,-8)$)}, scale=1, local bounding box=modelbox1]
		...
		\node[right = 2.5 * \boxdist of pretrain] (embed)  {};
		...
		\end{scope}
		\node[above=  of nn_box, align=center] (nn_text) {Long-Context Pre-training Language Model};
		...
		\node[\boxcolor, right = 1.5 * \boxdist of embed,  very thick, rounded corners, minimum size=\boxsize * 1.25] (weight_layer1) {};
		...
		% add weight layer 2
		\node[\boxcolor, right= \boxdist of add1, rectangle, very thick, rounded corners, minimum size=\boxsize * 1.25] (weight_layer2) {};
