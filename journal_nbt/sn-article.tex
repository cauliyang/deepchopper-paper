...
To refine the adapter sequence predictions, DeepChopper employs a sliding window and majority vote approach for each window in its post-processing stage~\figref{fig:f1}{c}.
After obtaining initial predictions for adapter locations, we apply a sliding window of 21 nucleotides with stride 1 across the read (See \hyperref[sec:methods]{Methods} for details).
Within each window, we analyze the distribution of predicted adapter positions to smooth potential irregularities and eliminate isolated false positives.
This refinement step helps ensure the predicted adapter sequences are contiguous and biologically plausible, improving the overall accuracy of adapter identification.
The sliding window strategy effectively filters out spurious predictions while preserving genuine adapter signals that show consistent patterns across adjacent windows.
...
Its efficient architecture, comprising just 7 million parameters, makes DeepChopper computationally practical for large-scale \gls{drs} analysis, unlike resource-intensive models such as Evo~\cite{nguyen2024sequence}, which requires billions of parameters.
