...
Given DeepChopper's ability to increase the yield of sequencing reads by correcting chimera artifacts, we reasoned that it could enhance transcript detection and quantification.
Using IsoQuant~\cite{prjibelski2023accurate} for long-read transcript annotation, we found that DeepChopper nearly doubled the number of identified transcripts compared to Dorado adapter-trimmed long-reads (\figref{fig:f2}{b}). 
This improvement was particularly evident in the detection of full-length transcripts, as shown by the increase in the \gls{fsm} category (\figref{fig:f2}{b}). 
Furthermore, DeepChopper improved the discovery of alternatively spliced transcripts, including those classified as \gls{ism}, \gls{nic}, and \gls{nnc}, further demonstrating its effectiveness in improving transcript diversity and accuracy in quantification.
