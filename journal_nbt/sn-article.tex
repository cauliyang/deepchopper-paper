...
For base calling, we used Dorado (v0.7.1) for both datasets~\cite{2024nanoporetech}.
...
The resulting fastq files were then aligned to the human reference genome (GRCh38) using Minimap2 (v2.26) with the \emph{-ax splice -uf -k7} configuration for direct RNA reads~\cite{li2018minimap2}.
...
Totally, we generated \num{120} million data points for training.
The dataset was split into training, validation, and test sets with ration \num{0.8}, \num{0.1}, and \num{0.1} respectively.
Then, we have \num{96} million data points for training, \num{12} million for validation, and \num{12} million for test.

\subsection{Tokenization}\label{ssec:tokenization}

We used a tokenization strategy that converts the biological sequences into tokens at the single nucleotide level.
...
\[
	\ell(x, y) =   \sum_{n=1}^N \frac{1}{\sum_{n=1}^N w_{y_n} \cdot \mathbb{1}\{y_n \not= \mathrm{ignore\_index}\}} l_n
	.\]
