...
To address these challenges, we developed DeepChopper, a genomic language model specifically designed for analyzing long-read sequences. Drawing inspiration from recent advances in \gls{llm} that have demonstrated the ability to decipher complex patterns in genetic sequences (cite: https://arxiv.org/html/2407.11435v1), DeepChopper is uniquely capable of processing long genomic contexts with single-nucleotide resolution. This capability is critical for accurately identifying ONT adapter sequences directly from base-called long-read sequences, thereby facilitating the detection of potential chimeric read artifacts in dRNA-seq data. DeepChopper processes individual FASTQ records to predict adapter sequences. When adapter sequences are detected in the middle of a long read (internal adapter), DeepChopper splits it into two or more FASTQ records (\figref{fig:f1}{a}). For adapter sequences found at the end of a read, DeepChopper removes the adapters, producing a single trimmed record. This approach ensures the preservation of genuine biological sequences while eliminating chimera artifacts. To accomplish this, DeepChopper employs a hybrid architecture that combines the \gls{lcglm} HyenaDNA~\cite{nguyen2024hyenadna} for efficient modeling of long-range dependencies with a residual block and \gls{mlp} for fine-grained sequence analysis, ensuring both broad context understanding and single-nucleotide resolution~\cite{poli2023hyena, he2016deep} (\figref{fig:f1}{a}). Additionally, DeepChopper integrates base quality score information alongside sequence data, enabling more accurate predictions that account for sequencing confidence. This quality-aware processing is complemented by an adaptive tokenization strategy that preserves single-nucleotide resolution while capturing higher-order sequence patterns.

DeepChopper offers several advantages over existing genomic language models. Unlike DNABERT2~\cite{zhou2023dnabert2}, which is limited to a context length of 512 base pairs, DeepChopper can process longer sequence inputs (up to 32 kilobase pairs), capturing important long-range dependencies. Additionally, unlike NuclideTransformer~\cite{dalla2023nucleotide}, which lacks single-base resolution, DeepChopper can accurately identify non-human reference sequences, such as ONT adapters. Moreover, DeepChopper's efficient architecture, with only 7 million parameters, makes it computationally practical for large-scale dRNA-seq analysis, unlike larger models like Evo~\cite{nguyen2024sequence} that may be resource-intensive.
