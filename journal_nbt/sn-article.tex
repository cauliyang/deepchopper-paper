...
DeepChopper introduces a hybrid architecture that combines a \gls{lcglm} HyenaDNA for efficient long-range dependency modeling with residual block and mlp for fine-grained sequence analysis, achieving both broad context understanding and single-nucleotide resolution~\cite{poli2023hyena, nguyen2024hyenadna} (\figref{fig:f1}{a}).
...
These results demonstrate ability of DeepChopper to significantly improve the quality of \gls{drs} data, enabling more accurate and reliable downstream analyses, such as quantification and gene fusion detection.
