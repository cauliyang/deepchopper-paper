...
DeepChopper approaches artificial sequence detection as a token classification task, predicting whether each token belongs to the adapter sequence.
...
By classifying tokens into positive (part of the adapter sequence) or negative (not part of the adapter sequence) categories, DeepChopper provides a nuanced analysis of the input data.
...
The model was trained using supervised learning, with sequences annotated for adapter sequences.
...
\[
	...
	\end{cases}
\]
where \( y_i \) is the final prediction for nucleotide \( x_i \). \( p_j \) is the predicted probability of being adapter sequence for the nucleotide \( x_j \). \( W \) is the size of the sliding window, \( k \) is half the window size, i.e., \( k = \frac{W-1}{2} \).

Following prediction refinement using a sliding window approach, we applied additional filters to enhance output quality.
Predicted adapter sequences shorter than 13 nucleotides were removed to minimize false positives due to noise.
Reads containing more than four predicted adapter sequences were excluded from the processing.
Finally, adapter sequences and their associated quality scores are removed, which converts one FASTQ record to multiple FASTQ records.
...
We collect the chimeric alignment and only select the chimeric alignment which includes two segments after aligning to the reference genome.
The result consisted of numerical values associated with 25 chromosomes (chromosome 1 to chromosome 22, chromosome X, chromosome Y and chromosome M).
...
Base quality scores were generated using a custom Python script, and ONT ion current signals were visualized with Squigualiser (v0.6.3)~\cite{samarakoon2024interactive}. The gene fusion events were visualized using chimeraviz (1.30.0)~\cite{laagstad2017chimeraviz}.
...
This project is supported in part by NIH grants R35GM142441 and R01CA259388 to RY. and NIH grant R01CA278832 to QC.

\bmhead{Author Contributions}

YL, TYW and RY designed the study with QC. YL and TYW performed the analysis. QG, YR and XL performed the experiments. YL designed the computational tool. YL, TYW, QG and RY wrote the manuscript. RY supervised this work.
...
\begin{figure}[!h]
	...
	\caption{ {\bf Analysis of Chimera artifact-derived gene fusions in \gls{drs}.} (a) Circos plot showing chromosomal connections of chimera artifact-derived fusion events in VCaP cells. Blue lines represent inter-chromosomal fusion events, while red lines indicate intra-chromosomal fusions. The outer track shows chromosomal ideograms labeled with respective chromosome numbers.
 
 
 Chromosomal connections of fusion events in false positive chimeric reads from VCaP 002 dataset. Red and blue links indicate intra and inter-chromosomal fusions. (b) \gls{go} enrichment of \gls{fp} chimeric reads detected by DeepChopper in HCT116, A549 and HepG2.}
	\label{fig:sf3}
\end{figure}
