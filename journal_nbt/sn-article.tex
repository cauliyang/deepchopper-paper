...
Using VCaP read subsamples from 0.1M to 9M reads, we observed near-linear runtime scaling (\mbox{\edfigref{fig:compute_benchmark}{a}}), with the full 9M VCaP dataset requiring approximately 5 hours on two NVIDIA A100 GPUs.
...
For incomplete 3' detection in multi-adapter reads, we recommend combining Dorado (3' end adapters) with DeepChopper (internal adapters) (\mbox{\edfigref{fig:challenge1solution}{}}).
...
\hl{To empirically assess Breakinator's applicability to \mbox{\gls{drs}}, we applied it to VCaP RNA002 data using the same cDNA-seq as validation(\mbox{\edfigref{fig:breakinator_comp}{}}).
While Breakinator reduced total chimeric alignments by 62\%, it decreased the cDNA support rate to 4.8\%---lower than baseline Dorado processing (5.8\%)---indicating indiscriminate removal of both artifacts and biological events.
In contrast, DeepChopper reduced unsupported chimeric alignments by 95\% while increasing cDNA support to 49\%, demonstrating selective artifact removal.
...
\begin{figure}[!ht]
   ...
   \hl{Representative read from VCaP RNA002, processed with Dorado trim option followed by DeepChopper, containing an internal adapter sequence (highlighted in red, positions 367-443). 
...
\end{figure}
  ...
  \begin{figure}[!ht]
    ...
    \end{center}
    \caption{{\bf Challenge Case 1: Incomplete 3' terminal adapter detection in multi-adapter read.} 
...
\begin{figure}[!ht]
    ...
    \end{center}
    \caption{{\bf Solution for Challenge Case 1: Combined Dorado-DeepChopper workflow.}
