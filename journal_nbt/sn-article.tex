...
The batch size was \num{64}, validation was performed every 20,000 \qx{The number should be revised throughout the MS}  steps, and we selected the model with the highest validation F1 score for the base prediction task.
The Adam optimizer was used for training with default \( \beta_{1} = 0.9 \) and \( \beta_{2} = 0.999 \) parameters~\cite{kingma2014adam}.
We use a learning rate scheduler to reduce the learning rate when the validation loss stops improving, and the initial learning rate is 0.00002.
...
\textit{Evaluation metrics and analysis.} We assessed DeepChopper's performance using precision, recall, and F1 score on testing data.
