...
\hl{Empirical comparison on VCaP RNA002: Breakinator reduced artifacts 62\% but decreased cDNA support to 4.8\% (below baseline 5.8\%), indicating indiscriminate removal (\mbox{\edfigref{fig:comp_with_breakinator}{}}). DeepChopper reduced artifacts 91\% while increasing cDNA support to 49\%. 
Distance rules cannot distinguish adapter-bridged artifacts from biological RNA rearrangements\mbox{~\cite{ma2018squid}} and they cannot handle artifacts that do not map as chimeric reads or that fail to map at all\mbox{(\edfigref{fig:non-chimeric-ratio}{})}.
Internal adapter presence definitively indicates artifacts, enabling selective correction while preserving biological complexity.}
...
\hl{Representative cases illustrate capabilities and limitations. \mbox{\edfigref{fig:adapter_example}{}} shows successful detection with characteristic poly-A patterns in the upstream and low base quality. 
...
\begin{figure}[!ht]
    ...
    \hl{Percentage of internal adapter-containing reads that do not produce chimeric alignments across five human cell lines (RNA002). 
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=\textwidth]{figures/finals/figure5e_adapter.pdf}
	\end{center}
	\caption{{\hl{\bf Representative internal adapter detection with base quality visualization.}}
		\hl{Representative read (\mbox{\figref{fig:f3}{e}}) from VCaP RNA002 processed with Dorado trim followed by DeepChopper, demonstrating internal adapter detection (highlighted in red, positions 155-229) and sub-reads recovery (blue box and purple box).
			(a)  BLAT alignment of upstream sequence (blue box, positions 1--154) to chr14 (RPS29 gene) with 0.98 identity, confirming genuine biological RNA.
			...
			(c) BLAT alignment of downstream sequence (purple box, positions 230--722) to chr11 (COX8A gene) with 0.96 identity, confirming genuine biological RNA from a different transcript.}
	}
