...
\hl{Performance benchmarking on the VCaP dataset demonstrated that DeepChopper processed all 9,177,639 reads in 5 hours and 2 minutes} (\edtableref{tab:st2}{}). \hl{Specifically, FASTQ to Parquet conversion took 25 minutes with peak memory usage of 40 GB, adapter prediction (batch size 64) completed in 3 hours and 42 minutes utilizing a maximum of 33 GB GPU and 3.7 GB CPU memory, and post-processing required 55 minutes with peak memory consumption of 41 GB. These benchmarks demonstrate that DeepChopper is practical for large-scale sequencing projects, capable of processing millions of reads within hours on standard high-performance computing infrastructure} (See \hyperref[sec:methods]{Methods} for details).
...
The final classification decision is based on the class with the higher probability score. Specifically, a nucleotide is classified as an adapter if $P(y_i = \textrm{adapter}) > 0.5$, as the probabilities for the two classes are complementary and sum to 1. }

\hl{This position-wise classification approach enables precise identification of adapter sequences at both terminal and internal positions within reads. By performing fine-grained nucleotide-level classification, DeepChopper accurately defines adapter boundaries, facilitating effective trimming across diverse direct RNA sequencing datasets.}
...
\begin{table}[htbp]
    \centering
    \caption{DeepChopper runtime and resource usage for processing VCaP \gls{drs} data with 9,177,639 raw reads}
    \begin{tabular}{lcccc}
        \toprule
