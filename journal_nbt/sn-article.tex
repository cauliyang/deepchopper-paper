...
\abstract{
	Chimera artifacts in nanopore \gls{drs} data can confound transcriptome analyses, yet no existing tools are capable of detecting and removing them due to limitations in basecalling models. We present DeepChopper, a genomic language model that accurately identifies and eliminates adapter sequences within base-called \gls{drs} reads, effectively removing chimeric read artifacts. DeepChopper significantly improves critical downstream analyses, including transcript annotation and gene fusion detection, enhancing the reliability and utility of nanopore \gls{drs} for transcriptomics research.
...
Using a MinION sequencer with four R9.4 flow cells, we generated a total of 9,177,639 long reads in FASTQ format, with base-calling performed by \gls{ont}'s Dorado software (\url{https://github.com/nanoporetech/dorado}).
...
Most of these adapter sequences were approximately 70 bp in length (\figref{fig:f1}{b}), aligning with the expected length of the RMX adapter used in \gls{ont} SQK-RNA002 \gls{drs} kit, as specified in the kit's technical documentation (\url{https://nanoporetech.com/document/chemistry-technical-document}).
...
Additionally, the LRGASP project generated cDNA-based long-read sequencing data from this cell line using distinct protocols (\gls{pcr}-cDNA, CapTrap, R2C2) on both \gls{ont} and \gls{pb} platforms, providing a valuable resource for benchmarking DeepChopper's performance.
...
\gls{go} enrichment analysis in VCaP (\figref{fig:f2}{d}) and SG-NEx cell lines (\edfigref{fig:sf3}{b}) confirmed that housekeeping ribosomal protein genes are frequently involved in these artifacts.
...
Subsequently, FASTQ files were generated using Dorado (\url{https://github.com/nanoporetech/dorado}, v0.5.2) with adapter trimming disabled (\texttt{--no-trim}) and the ``rna002\_70bps\_hac@v3'' model.
...
DeepChopper identifies adapter sequences by framing the problem as a token classification task. The model tokenizes biological sequences at the single-nucleotide level, treating each nucleotide (\emph{A}, \emph{C}, \emph{G}, \emph{T}, and \emph{N}) as a fundamental unit. This approach enables fine-grained sequence analysis crucial for distinguishing artificial adapter sequences from native biological sequences.

For each nucleotide token, DeepChopper predicts whether it belongs to an adapter sequence. The model's architecture is optimized for high-precision identification of adapter sequences within biological data. At its core, DeepChopper utilizes the HyenaDNA framework~\cite{nguyen2024hyenadna}, a specialized genomics architecture that serves as the primary feature extractor. This framework efficiently processes nucleotide sequences to produce rich feature representations with a dimensionality of 256.

These extracted features then pass through a quality block that integrates base quality scores. The quality block consists of dense layers with residual connections, incorporating sequence quality information to enhance the model's interpretation of the input data.

The enriched features are ultimately fed into a classification head that computes the probability of each token being part of an adapter sequence. Through binary classification at every nucleotide position, the model labels each token as either adapter or non-adapter sequence, providing a comprehensive analysis of the entire input sequence.

%DeepChopper identifies adapter sequences by framing the problem as a token classification task.
%Biological sequences are tokenized at the single-nucleotide level, where each nucleotide \emph{A}, \emph{C}, \emph{G}, \emph{T}, and \emph{N}, is treated as a fundamental unit.
%This approach enables the model to capture fine-grained sequence details crucial for distinguishing between adapter sequences and native biological sequences.

%For each nucleotide token, DeepChopper predicts whether it belongs to an adapter sequence or not.
%The model’s architecture is designed specifically to achieve high precision in identifying adapter sequences embedded within biological data.
%At the core of DeepChopper is the HyenaDNA framework~\cite{nguyen2024hyenadna}, a specialized genomics model architecture that serves as the primary feature extractor.
%This framework efficiently processes and interprets complex nucleotide sequences, producing rich feature representations.
%These extracted features, with a dimensionality of 256, are then passed through a quality block that integrates base quality scores into the prediction pipeline.
%The quality block is composed of dense layers with residual connections, strategically incorporating sequence quality information to further refine the model's understanding of the input data.

%Finally, the enriched feature set is fed into a classification head, which computes the probability of each token being part of an adapter sequence.
%The model performs a binary classification for every nucleotide position, labeling each token as either part of an adapter or a non-adapter sequence, thus providing a comprehensive analysis of the entire input sequence.
...
To identify the best hyperparameter configuration, the Hydra (\url{https://github.com/facebookresearch/hydra}, v1.3.2) framework was used.
...
Subsequently, FASTQ files were generated using Dorado (\url{https://github.com/nanoporetech/dorado}, v0.5.2) with adapter trimming enabled (\texttt{--trim} adapters) and the ``dna\_r9.4.1\_e8\_hac@v3.3'' model.
...
The raw FASTQ files (and FASTA files for \gls{ont} R2C2) from these datasets were provided by the LRGASP project~\cite{pardo2024systematic}.
...
For short-read RNA-seq data, FASTQ files for the VCaP cell line were obtained from the CCLE project~\cite{barretina2012cancer} under SRA accession SRX5417211.
...
\begin{thebibliography}{10}
    ...
    \bibinfo{author}{Garalde, D.~R.} \emph{et~al.}
    \newblock \bibinfo{title}{Highly parallel direct rna sequencing on an array of
        nanopores}.
    \newblock \emph{\bibinfo{journal}{Nature methods}}
    \textbf{\bibinfo{volume}{15}}, \bibinfo{pages}{201--206}
    (\bibinfo{year}{2018}).

    \bibitem{jain2022advances}
    \bibinfo{author}{Jain, M.}, \bibinfo{author}{Abu-Shumays, R.},
    \bibinfo{author}{Olsen, H.~E.} \& \bibinfo{author}{Akeson, M.}
    \newblock \bibinfo{title}{Advances in nanopore direct rna sequencing}.
    \newblock \emph{\bibinfo{journal}{Nature methods}}
    \textbf{\bibinfo{volume}{19}}, \bibinfo{pages}{1160--1164}
    (\bibinfo{year}{2022}).

    \bibitem{smith2020molecular}
    \bibinfo{author}{Smith, M.~A.} \emph{et~al.}
    \newblock \bibinfo{title}{Molecular barcoding of native rnas using nanopore
        sequencing and deep learning}.
    \newblock \emph{\bibinfo{journal}{Genome research}}
    \textbf{\bibinfo{volume}{30}}, \bibinfo{pages}{1345--1353}
    (\bibinfo{year}{2020}).

    \bibitem{liu2024sequencing}
    \bibinfo{author}{Liu-Wei, W.} \emph{et~al.}
    \newblock \bibinfo{title}{Sequencing accuracy and systematic errors of nanopore
        direct rna sequencing}.
    \newblock \emph{\bibinfo{journal}{BMC genomics}} \textbf{\bibinfo{volume}{25}},
    \bibinfo{pages}{528} (\bibinfo{year}{2024}).

    \bibitem{benegas2024genomic}
    \bibinfo{author}{Benegas, G.}, \bibinfo{author}{Ye, C.},
    \bibinfo{author}{Albors, C.}, \bibinfo{author}{Li, J.~C.} \&
    \bibinfo{author}{Song, Y.~S.}
    \newblock \bibinfo{title}{Genomic language models: Opportunities and
        challenges} (\bibinfo{year}{2024}).
    \newblock \urlprefix\url{https://arxiv.org/abs/2407.11435}.
    \newblock \eprint{2407.11435}.

    \bibitem{nguyen2024hyenadna}
    \bibinfo{author}{Nguyen, E.} \emph{et~al.}
    \newblock \bibinfo{title}{Hyenadna: Long-range genomic sequence modeling at
        single nucleotide resolution}.
    \newblock \emph{\bibinfo{journal}{Advances in neural information processing
            systems}} \textbf{\bibinfo{volume}{36}} (\bibinfo{year}{2024}).

    \bibitem{poli2023hyena}
    \bibinfo{author}{Poli, M.} \emph{et~al.}
    \newblock \bibinfo{title}{Hyena hierarchy: Towards larger convolutional
        language models}.
    \newblock \eprint{2302.10866}.

    \bibitem{he2015deep}
    \bibinfo{author}{He, K.}, \bibinfo{author}{Zhang, X.}, \bibinfo{author}{Ren,
        S.} \& \bibinfo{author}{Sun, J.}
    \newblock \bibinfo{title}{Deep {{Residual Learning}} for {{Image
                Recognition}}}.
    \newblock \eprint{1512.03385}.

    \bibitem{zhou2023dnabert2}
    \bibinfo{author}{Zhou, Z.} \emph{et~al.}
    \newblock \bibinfo{title}{{{DNABERT-2}}: Efficient foundation model and
        benchmark for multi-species genome}.
    \newblock \eprint{2306.15006}.

    \bibitem{dalla2023nucleotide}
    \bibinfo{author}{Dalla-Torre, H.} \emph{et~al.}
    \newblock \bibinfo{title}{The nucleotide transformer: Building and evaluating
        robust foundation models for human genomics}.
    \newblock \emph{\bibinfo{journal}{BioRxiv}} \bibinfo{pages}{2023--01}
    (\bibinfo{year}{2023}).

    \bibitem{nguyen2024sequence}
    \bibinfo{author}{Nguyen, E.} \emph{et~al.}
    \newblock \bibinfo{title}{Sequence modeling and design from molecular to genome
        scale with evo}.
    \newblock \emph{\bibinfo{journal}{bioRxiv}}  (\bibinfo{year}{2024}).
    \newblock
    \urlprefix\url{https://www.biorxiv.org/content/early/2024/02/27/2024.02.27.582234}.

    \bibitem{chen2021systematic}
    \bibinfo{author}{Chen, Y.} \emph{et~al.}
    \newblock \bibinfo{title}{A systematic benchmark of nanopore long read rna
        sequencing for transcript level analysis in human cell lines}.
    \newblock \emph{\bibinfo{journal}{BioRxiv}} \bibinfo{pages}{2021--04}
    (\bibinfo{year}{2021}).

    \bibitem{li2018minimap2}
    \bibinfo{author}{Li, H.}
    \newblock \bibinfo{title}{Minimap2: pairwise alignment for nucleotide
        sequences}.
    \newblock \emph{\bibinfo{journal}{Bioinformatics}}
    \textbf{\bibinfo{volume}{34}}, \bibinfo{pages}{3094--3100}
    (\bibinfo{year}{2018}).

    \bibitem{kent2002blat}
    \bibinfo{author}{Kent, W.~J.}
    \newblock \bibinfo{title}{Blat—the blast-like alignment tool}.
    \newblock \emph{\bibinfo{journal}{Genome research}}
    \textbf{\bibinfo{volume}{12}}, \bibinfo{pages}{656--664}
    (\bibinfo{year}{2002}).

    \bibitem{pardo2024systematic}
    \bibinfo{author}{Pardo-Palacios, F.~J.} \emph{et~al.}
    \newblock \bibinfo{title}{Systematic assessment of long-read rna-seq methods
        for transcript identification and quantification}.
    \newblock \emph{\bibinfo{journal}{Nature methods}} \bibinfo{pages}{1--15}
    (\bibinfo{year}{2024}).

    \bibitem{prjibelski2023accurate}
    \bibinfo{author}{Prjibelski, A.~D.} \emph{et~al.}
    \newblock \bibinfo{title}{Accurate isoform discovery with isoquant using long
        reads}.
    \newblock \emph{\bibinfo{journal}{Nature Biotechnology}}
    \textbf{\bibinfo{volume}{41}}, \bibinfo{pages}{915--918}
    (\bibinfo{year}{2023}).

    \bibitem{chen2023gene}
    \bibinfo{author}{Chen, Y.} \emph{et~al.}
    \newblock \bibinfo{title}{Gene fusion detection and characterization in
        long-read cancer transcriptome sequencing data with fusionseeker}.
    \newblock \emph{\bibinfo{journal}{Cancer research}}
    \textbf{\bibinfo{volume}{83}}, \bibinfo{pages}{28--33}
    (\bibinfo{year}{2023}).

    \bibitem{uhrig2021accurate}
    \bibinfo{author}{Uhrig, S.} \emph{et~al.}
    \newblock \bibinfo{title}{Accurate and efficient detection of gene fusions from
        rna sequencing data}.
    \newblock \emph{\bibinfo{journal}{Genome research}}
    \textbf{\bibinfo{volume}{31}}, \bibinfo{pages}{448--460}
    (\bibinfo{year}{2021}).
    ...
    \newblock \emph{\bibinfo{journal}{bioinformatics}}
    \textbf{\bibinfo{volume}{25}}, \bibinfo{pages}{2078--2079}
    (\bibinfo{year}{2009}).
    ...
    \bibinfo{author}{Li, Y.} \& \bibinfo{author}{Yang, R.}
    \newblock \bibinfo{title}{{PxBLAT: an efficient python binding library for
            BLAT}}.
    \newblock \emph{\bibinfo{journal}{BMC Bioinf.}} \textbf{\bibinfo{volume}{25}},
    \bibinfo{pages}{1--8} (\bibinfo{year}{2024}).

    \bibitem{martin2011cutadapt}
    \bibinfo{author}{Martin, M.}
    \newblock \bibinfo{title}{Cutadapt removes adapter sequences from
        high-throughput sequencing reads}.
    \newblock \emph{\bibinfo{journal}{EMBnet. journal}}
    \textbf{\bibinfo{volume}{17}}, \bibinfo{pages}{10--12}
    (\bibinfo{year}{2011}).

    \bibitem{grunberger2022nanopore}
    \bibinfo{author}{Gr{\"u}nberger, F.}, \bibinfo{author}{Ferreira-Cerca, S.} \&
    \bibinfo{author}{Grohmann, D.}
    \newblock \bibinfo{title}{Nanopore sequencing of rna and cdna molecules in
        escherichia coli}.
    \newblock \emph{\bibinfo{journal}{Rna}} \textbf{\bibinfo{volume}{28}},
    \bibinfo{pages}{400--417} (\bibinfo{year}{2022}).

    \bibitem{tardaguila2018sqanti}
    \bibinfo{author}{Tardaguila, M.} \emph{et~al.}
    \newblock \bibinfo{title}{Sqanti: extensive characterization of long-read
        transcript sequences for quality control in full-length transcriptome
        identification and quantification}.
    \newblock \emph{\bibinfo{journal}{Genome research}}
    \textbf{\bibinfo{volume}{28}}, \bibinfo{pages}{396--411}
    (\bibinfo{year}{2018}).

    \bibitem{barretina2012cancer}
    \bibinfo{author}{Barretina, J.} \emph{et~al.}
    \newblock \bibinfo{title}{The cancer cell line encyclopedia enables predictive
        modelling of anticancer drug sensitivity}.
    \newblock \emph{\bibinfo{journal}{Nature}} \textbf{\bibinfo{volume}{483}},
    \bibinfo{pages}{603--607} (\bibinfo{year}{2012}).

    \bibitem{dobin2013star}
    \bibinfo{author}{Dobin, A.} \emph{et~al.}
    \newblock \bibinfo{title}{Star: ultrafast universal rna-seq aligner}.
    \newblock \emph{\bibinfo{journal}{Bioinformatics}}
    \textbf{\bibinfo{volume}{29}}, \bibinfo{pages}{15--21}
    (\bibinfo{year}{2013}).

    \bibitem{hu2015gsds}
    \bibinfo{author}{Hu, B.} \emph{et~al.}
    \newblock \bibinfo{title}{Gsds 2.0: an upgraded gene feature visualization
        server}.
    \newblock \emph{\bibinfo{journal}{Bioinformatics}}
    \textbf{\bibinfo{volume}{31}}, \bibinfo{pages}{1296--1297}
    (\bibinfo{year}{2015}).

    \bibitem{samarakoon2024interactive}
    \bibinfo{author}{Samarakoon, H.} \emph{et~al.}
    \newblock \bibinfo{title}{Interactive visualisation of raw nanopore signal data
        with squigualiser}.
    \newblock \emph{\bibinfo{journal}{Biorxiv}} \bibinfo{pages}{2024--02}
    (\bibinfo{year}{2024}).

    \bibitem{laagstad2017chimeraviz}
    \bibinfo{author}{L{\aa}gstad, S.} \emph{et~al.}
    \newblock \bibinfo{title}{chimeraviz: a tool for visualizing chimeric rna}.
    \newblock \emph{\bibinfo{journal}{Bioinformatics}}
    \textbf{\bibinfo{volume}{33}}, \bibinfo{pages}{2954--2956}
    (\bibinfo{year}{2017}).

    \bibitem{sherman2022david}
    \bibinfo{author}{Sherman, B.~T.} \emph{et~al.}
    \newblock \bibinfo{title}{David: a web server for functional enrichment
        analysis and functional annotation of gene lists (2021 update)}.
    \newblock \emph{\bibinfo{journal}{Nucleic acids research}}
    \textbf{\bibinfo{volume}{50}}, \bibinfo{pages}{W216--W221}
    (\bibinfo{year}{2022}).
