...
Importantly, chimeric read artifacts frequently contain internal adapter sequences\mbox{~\cite{smith2020molecular}}, suggesting that these adapter-bridged chimeras could theoretically be distinguished from biological chimeras by detecting the presence of internal adapters.
...
Biologically, human protein-coding transcripts have a median length of approximately 2.7 kb (95th percentile: ${ \sim }8.7$ kb, 99th percentile: ${ \sim }14$ kb), with over 99.97\% of transcripts falling below 32 kb based on current annotations (\mbox{\edfigref{fig:transcript_len}{}}).
...
In addition to adapter-trimming tools, we also evaluated Breakinator\mbox{~\cite{heinz2025detecting}}, which is designed to detect chimeric artifacts rather than adapter boundaries.
Breakinator was benchmarked separately using real VCaP RNA002 \mbox{\gls{drs}} data.
As shown in \mbox{\edfigref{fig:comp_with_breakinator}{}}, Breakinator reduced detected artifacts by 62\% but simultaneously decreased cDNA support to 4.8\% (baseline 5.8\%), reflecting substantial loss of biologically supported signal.
...
A substantial fraction of \mbox{\gls{drs}} artifacts do not appear as chimeric alignments or fail to map (\mbox{\edfigref{fig:non-chimeric-ratio}{}}), making them inherently undetectable by alignment-rule–based chimera callers.
...
We systematically benchmarked DeepChopper's computational performance to assess scalability for large-scale \mbox{\gls{drs}} studies, testing datasets ranging from 0.1M to 23M reads.
...
It identified 8,218,172 adapter sequences across 7,990,102 reads (87\% of total), most measuring ${\sim}70$ bp, consistent with the expected length of the RMX adapter used in \gls{ont}'s SQK-RNA002 \gls{drs} kit (\figref{fig:f2}{a})~\cite{nano2017tech}.
Analysis of adapter locations revealed that 7,777,624 reads had adapters at the $3'$ end, while 148,452 contained internal adapters (\figref{fig:f2}{b}), indicating that chimeric artifacts are common in VCaP \gls{drs} data.
...
DeepChopper detected internal adapters in 0.67-1.25\% of reads across these datasets (A549: 0.92\%, MCF7: 0.67\%, HCT116: 1.22\%, K562: 0.96\%, HepG2: 1.25\%), representing 15,690-57,122 affected reads per sample (\mbox{\edtableref{tab:prevalence}{}}).
...
DeepChopper selectively eliminated only those chimeric alignments not supported by any cDNA-based method (\figref{fig:f2}{h}), further demonstrating its precision in distinguishing genuine chimeras from technical artifacts.
...
DeepChopper reliably removed artifactual chimeric reads not supported by any orthogonal cDNA-based sequencing platform (\edfigref{fig:mouse_chimera}{}), confirming its applicability to both human and non-human transcriptomes.
...
In zero-shot application, DeepChopper detected internal adapters in 0.33\% of VCaP RNA004 reads (38,878 reads out of 11,714,520 total), lower than the 1.62\% observed in VCaP RNA002 (\mbox{\edtableref{tab:prevalence}{}}).
...
To optimize performance on RNA004 data, we fine-tuned DeepChopper using a dataset with 300,000 reads created from VCaP RNA004 reads (70\% training, 20\% validation, 10\% test) (See \mbox{\hyperref[sec:methods]{Methods}} for details).
...
While DeepChopper's reduction of chimeric alignments in RNA004 (21-22\%) is lower than in RNA002 (91\%), both the reduced artifact prevalence (0.33\% vs 1.62\%) and the lower reduction magnitude are expected given chemistry improvements designed to reduce artifact formation\mbox{~\cite{hewel2024direct,zou2025comparative}}.
...
\mbox{\edfigref{fig:adapter_example}{}} illustrates the corresponding sequence and base qualities, including the alignment of upstream and downstream segments around the detected adapter, confirming that the apparent \textit{RPS29–COX8A} fusion arises from an adapter-bridged chimera.
...
DeepChopper addresses a critical gap in \mbox{\gls{ont}} \mbox{\gls{drs}}: detecting and recovering adapter-bridged chimeric artifacts through internal adapter identification.
...
DeepChopper leverages \mbox{\gls{glm}} capabilities unavailable to conventional approaches: (1) Long-range context\mbox{~\cite{nguyen2024hyenadna}} enabling full-transcript scanning (median ${ \sim }$2.7 kb, 99th percentile ${ \sim }$14 kb); (2) Transfer learning enabling robust detection despite sequencing errors; (3) Quality-aware predictions integrating per-base confidence (F1: $0.97\rightarrow0.99$); (4) Alignment-free operation eliminating mapping biases.

Performance differences between DeepChopper and existing tools reflect capability gaps, not quality.
Each tool excels at its intended application: Pychopper for cDNA primer detection\mbox{~\cite{pychopper,sessegolo2019transcriptome}} and Dorado for $3'$ adapter trimming\mbox{~\cite{dorado2023}}.
However, none detect internal adapters in \mbox{\gls{drs}}.
Pychopper targets cDNA-specific primers absent in \mbox{\gls{drs}}.
Porechop/Porechop\_ABI require exact matching or stable k-mer patterns\mbox{~\cite{Wick2017,bonenfant2023porechop}}, an approach incompatible with unknown, corrupted adapters mis-called by RNA basecallers.
...
Distance rules cannot distinguish adapter-bridged artifacts from biological RNA rearrangements\mbox{~\cite{ma2018squid}} and they cannot handle artifacts that do not map as chimeric reads or that fail to map at all.
...
Validation across RNA002 and RNA004 with Dorado basecalling shows adapter-bridged chimeras persist systematically despite chemistry improvements \mbox{~\cite{hewel2024direct,zou2025comparative}}.
...
To empirically assess the applicability of alignment-based artifact detection methods to \mbox{\gls{drs}}, we compared DeepChopper with Breakinator (v1.0)\mbox{~\cite{heinz2025detecting}} using VCaP RNA002 data.
...
\begin{figure}[!ht]
	...
	\caption{{\bf Representative internal adapter detection with base quality visualization.}
		Representative read (\mbox{\figref{fig:f3}{e}}) from VCaP RNA002 processed with Dorado trim followed by DeepChopper, demonstrating internal adapter detection (highlighted in red, positions 155-229) and sub-reads recovery (blue box and purple box).
...
\begin{figure}[!ht]
	...
	\caption{{\bf Solution for Challenge Scenario 1: Combined Dorado-DeepChopper workflow.}
		...
		This demonstrates that combining Dorado (3' end adapters) with DeepChopper (internal adapters) addresses complementary problems and resolves the incomplete detection issue shown in Challenge Scenario 1. (\mbox{\edfigref{fig:challenge1}{})}}
