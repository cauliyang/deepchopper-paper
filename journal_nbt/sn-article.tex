...
\hl{In addition to adapter-trimming tools, we also evaluated Breakinator\mbox{~\cite{heinz2025detecting}}, which is designed to detect chimeric artifacts rather than adapter boundaries.
...
\hl{\mbox{\edfigref{fig:adapter_example}{}} illustrates the corresponding sequence and base qualities, including the alignment of upstream and downstream segments around the detected adapter, confirming that the apparent \textit{RPS29–COX8A} fusion arises from an adapter-bridged chimera.}
...
\hl{Performance differences between DeepChopper and existing tools reflect capability gaps, not quality.
...
\hl{Pychopper targets cDNA-specific primers absent in \mbox{\gls{drs}}.
...
\hl{Challenging scenarios (\mbox{\edfigref{fig:challenge1}{}}, \mbox{\edfigref{fig:challenge2}{}}) include incomplete $3'$ end detection in multi-adapter reads and partial detection of degraded sequences. Solutions include combining Dorado ($3'$ end adapters) with DeepChopper (internal adapters) (\mbox{\edfigref{fig:challenge1solution}{}}) and post-processing length filtering (default 20 bp minimum).
...
\hl{Hence, a threshold of 0.5 is applied implicitly.}
...
\begin{figure}[!ht]
	...
	\end{center}
	\caption{{\hl{\bf Percent of internal adapter-containing reads that are non-chimeric}}
		\hl{Percentage of internal adapter-containing reads that do not produce chimeric alignments across five human cell lines (RNA002) processed by Dorado with trim followed by DeepChopper.
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=\textwidth]{figures/finals/figure3e_adapter.pdf}
	\end{center}
	\caption{{\hl{\bf Representative internal adapter detection with base quality visualization.}}
		\hl{Representative read (\mbox{\figref{fig:f3}{e}}) from VCaP RNA002 processed with Dorado trim followed by DeepChopper, demonstrating internal adapter detection (highlighted in red, positions 155-229) and sub-reads recovery (blue box and purple box).
...
\begin{figure}[!ht]
	...
	\end{center}
	\caption{{\hl{\bf Challenge Scenario 1: Incomplete 3' terminal adapter detection in multi-adapter read.}}
...
\begin{figure}[!ht]
	...
	\end{center}
	\caption{{\hl{\bf Challenge Scenario 2: Partial internal adapter detection.}}
...
\begin{figure}[!ht]
	...
	\end{center}
	\caption{{\hl{\bf Solution for Challenge Scenario 1: Combined Dorado-DeepChopper workflow.}}
