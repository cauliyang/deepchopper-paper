...
\abstract{
	The accurate identification of chimeric reads is critical for ensuring the reliability of nanopore \gls{drs} data, which has emerged as a powerful tool for sequencing full-length RNA molecules while preserving native RNA modifications. Despite its potential, the nanopore \gls{drs} is susceptible to the generation of chimeric reads, where fragments of distinct RNA molecules are incorrectly joined, complicating downstream analyzes such as transcriptome assembly and gene fusion detection.

	In this study, we present DeepChopper, a genomic language model specifically designed to detect and filter chimeric reads. Leveraging advanced long-range attention mechanisms and tailored tokenization strategies, DeepChopper effectively distinguishes true biological sequences from artificial constructs with high precision. We validated the performance of DeepChopper in multiple data sets, including \gls{ont} \gls{pcr} cDNA, \gls{ont} CapTrap-seq, \gls{ont} \gls{r2c2}, \gls{pb} cDNA, and \gls{pb} CapTrap-seq.

	Our results demonstrate that DeepChopper significantly outperforms existing methods, achieving up to a 94\% reduction in unsupported reads and consistently maintaining high support rates across various sequencing platforms\qx{such as ONT PCR cDNA, ONT CapTrap-seq, ONT R2C2, PacBio cDNA, and PacBio CapTrap-seq}. Furthermore, DeepChopper effectively identified low-quality artificial sequences based on the base quality score and the identity of \gls{blat}, further improving the precision of the nanopore \gls{drs} data.

	This study highlights the potential of DeepChopper to improve the integrity of nanopore \gls{drs} data, paving the way for more reliable genomics analyses and broadening the applicability of genomic language models in bioinformatics.}
...
\subsection{Gene fusion detection and visualization}

Gene fusions were identified using Fusionseeker (v1.0.1)~\cite{chen2023} in Dorado-trimmed reads, Deepchopper-processed reads, and FP chimeric reads. The gene structure was visualized using GSDS (v2.0)~\cite{hu2015}. Base quality scores were generated using a custom Python script, and ONT ion current signals were visualized with Squigualiser (v0.6.3)~\cite{Samarakoon2024}.


\ty{Chen, Y. et al. Gene Fusion Detection and Characterization in Long-Read Cancer Transcriptome Sequencing Data with FusionSeeker. Cancer Res 83, 28-33 (2023).
Hu, B. et al. GSDS 2.0: an upgraded gene feature visualization server. Bioinformatics 31, 1296-7 (2015).
Samarakoon, H. et al. Interactive visualisation of raw nanopore signal data with Squigualiser. bioRxiv, 2024.02.19.581111 (2024).}
