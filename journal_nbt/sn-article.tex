...
\hl{Importantly, chimeric read artifacts frequently contain internal adapter sequences\mbox{~\cite{smith2020molecular}}, suggesting that these adapter-mediated chimeras could theoretically be distinguished from biological chimeras by detecting the presence of internal adapters.}
...
As a result, current adapter detection tools\mbox{~\cite{pychopper, Wick2017, bonenfant2023porechop}} cannot exploit this feature to eliminate these adapter-mediated chimeras, leaving the issue unresolved (\mbox{\edtableref{tab:tools}{}}).
...
\hl{Biologically, human protein-coding transcripts have a median length of approximately 1.9 kb (95th percentile: $\sim5.8$ kb, 99th percentile: $\sim8.7$ kb), with over 99.99\% of transcripts falling below 32 kb based on current annotations (\mbox{\edfigref{fig:transcript_len}{}}).
...
Using VCaP read subsamples from 0.1M to 9M reads, we observed near-linear runtime scaling (\mbox{\edfigref{fig:sf2a}{a}}), with the full 9M VCaP dataset requiring approximately 5 hours on two NVIDIA A100 GPUs.
Memory usage increased with input size, peaking at approximately 70 GB for CPU-based inference and 56 GB for GPU-based execution (\mbox{\edfigref{fig:sf2a}{b}}).
...
This systematic occurrence of internal adapters across all tested cell lines indicates that adapter-mediated chimeras are not specific to VCaP but represent a general characteristic of dRNA-seq data.}
...
\hl{In zero-shot application, DeepChopper detected internal adapters in 0.33\% of VCaP RNA004 reads (38,878 reads out of 11,714,520 total), substantially lower than the 1.62\% observed in VCaP RNA002 (\mbox{\edtableref{tab:prevalence}{}}).}
DeepChopper reduced chimeric alignments by 21\% compared to Dorado base-called and adapter-trimmed reads, increasing the proportion of cDNA-supported chimeric alignments from approximately 25\% to 30\% (\edfigref{fig:sf3}{a}).
...
Internal adapter-like sequences identified by DeepChopper exhibited low base quality scores (mean Q-score: 7.8) and poor alignment identity to the human genome (mean \gls{blat} identity: 0.38), supporting their classification as artifacts (\edfigref{fig:sf3}{b}).
...
The fine-tuned model achieved marginal additional improvement, reducing chimeric alignments by 24-27\% compared to Dorado-processed reads---a 3-6\% improvement over the zero-shot model (\mbox{\edfigref{fig:finetune}{}}).
Critically, both the original RNA002-trained model and the RNA004-fine-tuned model preserved all cDNA-supported chimeric alignments, demonstrating that DeepChopper specifically targets adapter-mediated artifacts rather than biological structural variations.}

\hl{While DeepChopper's reduction of chimeric alignments in RNA004 (21-27\%) is lower than in RNA002 (91\%), both the reduced artifact prevalence (0.33\% vs 1.62\%) and the lower reduction magnitude are expected given chemistry improvements designed to reduce artifact formation\mbox{~\cite{hewel2024direct,zou2025comparative}}.
Nonetheless, the systematic detection of internal adapters across both RNA002 and RNA004 chemistries confirms that adapter-mediated chimeras remain an inherent characteristic of current dRNA-seq workflows, and DeepChopper's ability to generalize across chemistries without retraining highlights its robustness for emerging platforms.}
...
This enrichment extended to chimera artifacts in RNA004 data as well (\edfigref{fig:sf5}{b}).
...
In summary, DeepChopper significantly improves the quality of nanopore \gls{drs} data by accurately identifying and splitting adapter-mediated chimera that otherwise confound transcript annotation and gene fusion detection.
These improvements are robust across different sequencing chemistries and preprocessing pipelines.

\section{\hl{Discussion}}

\hl{DeepChopper addresses a critical gap in \mbox{\gls{ont}} \mbox{\gls{drs}}: detecting and recovering adapter-mediated chimeric artifacts through internal adapter identification.
Our validation across multiple cell lines, species, and chemistries demonstrates that internal adapters occur systematically in 0.33-1.62\% of reads---tens of thousands to hundreds of thousands of artifacts per experiment---with each propagating errors throughout downstream transcript annotation, gene fusion detection, and structural variant identification\mbox{~\cite{ma2018squid}}.}

\hl{DeepChopper's core principle—identifying internal adapters marking chimeric junctions—fundamentally differs from conventional quality control.
Rather than filtering problematic reads, it splits chimeras at adapter boundaries with single-nucleotide precision, recovering component sub-reads.
Representative cases illustrate this: \mbox{\edfigref{fig:adapter_example}{}} shows successful detection with characteristic low base quality and poly-A patterns, while challenging cases (\mbox{\edfigref{fig:challenge1}{}}, \mbox{\edfigref{fig:challenge2}{}}) reveal scenarios with multiple adapters or highly degraded sequences.
For incomplete 3' detection in multi-adapter reads, we recommend combining Dorado (3' end adapters) with DeepChopper (internal adapters).
For partial detections, post-processing length filtering (default 20 bp minimum) removes spurious fragments while preserving biological sequences.}

\hl{This recovery mechanism depends on detectable internal adapter sequences, distinguishing adapter-mediated chimeras from other artifact types.
\mbox{\gls{rt}}-mediated template-switching in cDNA-seq creates chimeric molecules through direct enzymatic joining without internal adapters\mbox{~\cite{schulz2021direct,balazs2019templateswitching,garalde2018highly,jain2022advances,zou2025comparative}}, representing a fundamentally different artifact class requiring alternative detection strategies.}

\hl{DeepChopper leverages \mbox{\gls{glm}} for capabilities conventional approaches cannot provide.
HyenaDNA's long-range context enables scanning entire full-length transcripts, while transfer learning from large-scale genomic pre-training enables robust detection of corrupted adapter patterns.
Our quality block integrates base-call confidence for adaptive predictions across variable-quality regions.
Critically, alignment-free operation eliminates systematic biases from mapping failures, multi-mapping reads, and novel sequences.
The sliding window refinement evaluates each position independently based on surrounding context rather than iteratively propagating predictions, preventing error cascades that would distort adapter boundaries—essential for accurate sub-read recovery.}

\hl{The dramatic performance differences between DeepChopper and existing tools (\mbox{\edtableref{tab:tools}{}}, \mbox{\edfigref{fig:benchwithtools}{}}) reflect fundamental capability gaps rather than tool quality.
Pychopper targets cDNA-specific primers absent in \mbox{\gls{drs}}\mbox{~\cite{pychopper}}, Porechop and Porechop\_ABI rely on exact sequence matching or k-mer frequency analysis that fail on error-prone \mbox{\gls{drs}} data\mbox{~\cite{Wick2017,bonenfant2023porechop}}, and Dorado excels at 3' end trimming but cannot detect internal adapters\mbox{~\cite{dorado2023}}.
Each tool demonstrates strong performance for its intended application—the negligible performance on internal adapter detection confirms this represents a previously unsolved problem requiring purpose-built solutions.}

\hl{Recently developed alignment-based methods, including Breakinator\mbox{~\cite{heinz2025detecting}} and FLAIR-fusion\mbox{~\cite{felton2023detection}}, employ symptom-based filtering—identifying reads with aberrant alignment patterns and removing them entirely.
Breakinator detects \mbox{\gls{rt}}-mediated foldback inversions in cDNA-seq using hand-crafted distance rules; its validation confirms these fail in \mbox{\gls{drs}} where \mbox{\gls{rt}}-mediated artifacts do not occur\mbox{~\cite{heinz2025detecting}}.
FLAIR-fusion is a specialized gene fusion caller, not a general quality control tool.
Critically, alignment-based distance rules cannot distinguish adapter-mediated artifacts from genuine biological structural variations like trans-splicing or circular RNAs.
DeepChopper's mechanism-based detection provides unambiguous identification: internal adapter presence definitively indicates library preparation or basecalling artifacts, enabling selective removal while preserving biological complexity.}

\hl{Our validation spans RNA002 and RNA004 chemistries with updated Dorado basecalling, demonstrating that adapter-mediated chimeras persist systematically across both, though at reduced prevalence in RNA004 (0.33\% versus 1.62\% in matched VCaP cells).
This reduction reflects chemistry improvements\mbox{~\cite{hewel2024direct,zou2025comparative}}, yet systematic occurrence across all tested conditions indicates adapter-mediated chimera formation remains inherent to current \mbox{\gls{drs}} workflows.
Fine-tuning DeepChopper on RNA004 achieved marginal additional improvement (3-6\% beyond the original model's cross-chemistry generalization, \mbox{\edfigref{fig:finetune}{}}), with both models preserving all cDNA-supported biological chimeras.
This demonstrates that \mbox{\gls{glm}}' learned probabilistic patterns enable robust generalization without requiring complete retraining for each technology iteration.
Importantly, no peer-reviewed literature has systematically characterized adapter-mediated chimera formation mechanisms in \mbox{\gls{drs}}---our work addresses this knowledge gap while providing computational solutions.}

\hl{DeepChopper's applicability to other platforms depends on chimera formation mechanisms.
For \mbox{\gls{ont}} cDNA-seq, \mbox{\gls{rt}}-mediated template-switching creates chimeric molecules without internal adapters\mbox{~\cite{schulz2021direct,balazs2019templateswitching}}, making them unsuitable for adapter-based detection.}

\hl{DeepChopper exemplifies how \mbox{\gls{glm}} can address technical challenges in long-read sequencing that resist conventional computational approaches.
The combination of long-range contextual understanding, error-tolerant pattern learning, single-nucleotide precision, and quality-aware predictions enables detection of corrupted internal adapters that elude exact-matching, k-mer-based, or alignment-based methods.
By addressing adapter-mediated chimeras through recovery rather than removal, DeepChopper represents a philosophical shift from artifact filtering to artifact correction, preserving valuable information while enhancing accuracy.
This advance enables more confident biological interpretation of \mbox{\gls{drs}} data across diverse applications—from transcript isoform discovery to gene fusion detection—ultimately strengthening the foundation for transcriptomic research in complex biological systems and disease contexts.}
...
The fine-tuning dataset consisted of 300,000 reads generated from VCaP RNA004, split into training (70\%, 210,000 reads), validation (20\%, 60,000 reads), and test (10\%, 30,000 reads) sets.
In each dataset, it consists of positive examples containing adapter sequences (with a 1:1 ratio of $3'$ end and internal adapters) and negative examples without any adapter sequences, in a 9:1 ratio.
...
Training was performed on two NVIDIA A100 GPUs using the same optimization settings described in \hyperref[ssec:training]{Model training}. 
Performance evaluation compared three conditions: (1) Dorado basecalling with adapter trimming, (2) Dorado followed by the original RNA002-trained DeepChopper, and (3) Dorado followed by the RNA004-fine-tuned DeepChopper. 
...
\begin{figure}[!ht]
    ...
    \hl{Analysis of all protein-coding transcripts from Ensembl GRCh38.115 (released July 2025) shows that $>$99.99\% of transcripts are below the 32 kb threshold (marked with vertical dashed line). The distribution is highly skewed toward shorter transcripts, with median length of $\sim$1.9 kb.}}
...
\begin{table}[!ht]
\centering
\caption{\hl{Internal Adapter Prevalence by Sample}}
\label{tab:prevalence}
% \setlength{\tabcolsep}{6pt}
\begin{tabular}{lccc}
\toprule
\textbf{Sample} & \textbf{Internal Adapters} & \textbf{Total Reads} & \textbf{Fraction (\%)} \\
\midrule
A549 & 15,690 & 1,703,697 & 0.92 \\
MCF7 & 20,340 & 3,039,468 & 0.67 \\
HCT116 & 57,122 & 4,697,299 & 1.22 \\
K562 & 29,436 & 3,061,722 & 0.96 \\
HepG2 & 22,530 & 1,797,922 & 1.25 \\
VCaP RNA002 & 148,452 & 9,177,422 & 1.62 \\
VCaP RNA004 & 38,878 & 11,714,520 & 0.33 \\
\bottomrule
\end{tabular}
\footnotetext{Total reads from Dorado with trim. Internal adapters detected by DeepChopper after Dorado processing. 
VCaP RNA002 and RNA004 demonstrate that adapter-mediated chimeras persist across chemistries.}
\end{table}
...
\begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=\textwidth]{finals/cd57397b-5cb3-49bb-aa54-b672ead44527_seq_qual}
    \end{center}
    \caption{{\bf Internal adapter detection with base quality visualization.} 
    \hl{Representative dRNA-seq read from VCaP RNA002 containing an internal adapter sequence (highlighted in red, positions 364-438). Base quality scores ($Q$ scores) shown as color-coded bars: yellow indicates high quality ($Q>40$), dark blue indicates low quality ($Q<10$). The adapter region shows characteristic poly-A sequences upstream and lower base quality compared to flanking biological sequences. Sequences before and after the adapter represent genuine biological RNA from different transcripts artificially joined during library preparation or basecalling. Read ID: cd57397b-5cb3-49bb-aa54-b672ead44527.}}
...
\begin{figure}[!ht]
    ...
    \end{center}
    \caption{{\bf Challenge Case 1: Incomplete 3' adapter detection in multi-adapter read.} 
    Representative read containing both internal adapter (positions 1026-1105, highlighted in red, correctly detected) and 3' end adapter that DeepChopper failed to completely detect. 
    ...
    The internal adapter shows characteristic low quality. Read ID: c16c6ade-135b-4073-a1d6-5a9c6900bfb2.}
    \label{fig:challenge1}
\end{figure}
    
\begin{figure}[!ht]
    \begin{center}
        \includegraphics[width=\textwidth]{finals/1deaffc6-3a21-45d4-a875-d9c028ce695e_seq_qual}
    \end{center}
    \caption{{\bf Challenge Case 2: Partial internal adapter detection.} 
    Read showing partial adapter detection (highlighted in red). DeepChopper detected partial internal adapter sequence, demonstrating challenges with degraded or error-rich adapter sequences.
    Base quality scores ($Q$ scores) shown as color-coded bars: yellow indicates high quality ($Q>40$), dark blue indicates low quality ($Q<10$).            
    Read ID: 1deaffc6-3a21-45d4-a875-d9c028ce695e.}
