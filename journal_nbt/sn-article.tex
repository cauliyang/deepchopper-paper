...
Critically, both the original RNA002-trained model and the RNA004-fine-tuned model preserved all cDNA-supported chimeric alignments, demonstrating that DeepChopper specifically targets adapter-mediated artifacts rather than biological RNA rearrangements\mbox{\cite{ma2018squid}}.}
...
\hl{DeepChopper addresses a critical gap in \mbox{\gls{ont}} \mbox{\gls{drs}}: detecting and recovering adapter-mediated chimeric artifacts through internal adapter identification. Our validation across multiple cell lines, species, and chemistries demonstrates internal adapters occur systematically in 0.33-1.62\% of reads---tens to hundreds of thousands per experiment---with each propagating errors to transcript annotation, gene fusion detection, and expression quantification\mbox{~\cite{ma2018squid}}. No prior literature has systematically characterized adapter-mediated chimera formation mechanisms in \mbox{\gls{drs}}; our work addresses this gap while providing the first computational solution.}
\hl{DeepChopper identifies internal adapter sequences marking artificial junctions, then splits chimeras at these boundaries with single-nucleotide precision to recover component sub-reads. This mechanism-based approach differs fundamentally from filtering methods that discard problematic reads.
The recovery capability depends on detectable internal adapters, distinguishing adapter-mediated artifacts from \mbox{\gls{rt}}-mediated template-switching in cDNA-seq, which creates chimeras without internal adapters\mbox{~\cite{schulz2021direct,balazs2019templateswitching}}.}

\hl{DeepChopper leverages \mbox{\gls{glm}} capabilities unavailable to conventional approaches: (1) Long-range context\mbox{~\cite{nguyen2024hyenadna}} enabling full-transcript scanning (median $\sim$1.9 kb, 99th percentile $\sim$8.7 kb); (2) Transfer learning enabling robust detection despite sequencing errors; (3) Quality-aware predictions integrating per-base confidence (F1: $0.97\rightarrow0.99$, \mbox{\edtableref{tab:ablation}{}}); (4) Alignment-free operation eliminating mapping biases; (5) Independent position evaluation preventing error cascades that would distort boundaries.}
...
\hl{Alignment-based methods employ symptom-based filtering---flagging aberrant alignment patterns---rather than detecting root causes.
...
\begin{tabular}{lccc}
\toprule
\textbf{Sample} & \textbf{Reads with Internal Adapters} & \textbf{Total Reads} & \textbf{Fraction (\%)} \\
\midrule
