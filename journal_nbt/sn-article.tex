...
\hl{As a result, current adapter detection tools\mbox{~\cite{pychopper, Wick2017}} cannot exploit this feature to eliminate chimeric read artifacts, leaving the issue unresolved} (\edtableref{tab:st1}{}).
...
The model applies a 21-nucleotide sliding window with a stride of 1 across the read, analyzing the distribution of predicted adapter positions within each window (See \hyperref[sec:methods]{Methods} for details).
...
\hl{DeepChopper employs a three-stage pipeline for adapter prediction. First, FASTQ inputs are converted to the Parquet format to enhance computational efficiency. Second, the core model predicts adapter sequences. Third, post-processing refines these predictions. We implemented computationally intensive components in Rust while developing the neural network architecture in Python.}

\hl{We optimized computational efficiency through multiple strategies: (1) utilizing Parquet format for enhanced data access speed, (2) implementing computationally intensive components in Rust programming language, (3) leveraging GPU acceleration for neural network inference, and (4) employing parallelization techniques across pre-processing and post-processing operations.}

\hl{Performance evaluation on a VCaP cell line dataset (9,177,639 reads) demonstrated that FASTQ to Parquet conversion required an average of 25 minutes with peak memory usage of 40 GB. The adapter prediction stage, executed with a batch size of 64, completed in approximately 3 hours and 42 minutes, utilizing a maximum of 33 GB GPU and 3.7 GB CPU memory. Post-processing required an average of 55 minutes with peak memory consumption of 41 GB. In total, DeepChopper processed the entire dataset in 5 hours and 2 minutes.} (\edtableref{tab:st2}{})

\hl{These benchmarks demonstrate that DeepChopper is practical for large-scale sequencing projects, capable of processing millions of reads within hours on standard high-performance computing infrastructure. The memory requirements and processing times are reasonable considering the complexity of the task and the volume of data being processed.}
...
\hl{The processed sequence features are subsequently fed into a classification head consisting of a two-layer neural network. This specialized architecture transforms the feature representations into classification outputs for each nucleotide position.
The classification head employs a softmax activation function to compute probability distributions across two classes: adapter and non-adapter.
...
For each nucleotide position, the softmax function generates normalized probabilities that sum to 1, effectively quantifying the model's confidence in its classification decision.
Our approach selects the class with the higher probability. In other words, a nucleotide is classified as an adapter if $P(y_i = \textrm{adapter}) > P(y_i = \textrm{non-adaper})$. Since these probabilities are complementary (they sum to 1), this is mathematically equivalent to classifying a position as an adapter when $P(y_i = \textrm{adapter}) > 0.5$.}


\hl{Through this binary classification approach, DeepChopper performs fine-grained discrimination at each nucleotide position by selecting the class with the higher probability score as the final prediction.
This strategy enables precise boundary detection between adapter and non-adapter regions, facilitating comprehensive adapter identification throughout the input sequence.
The position-wise classification paradigm allows DeepChopper to effectively handle both terminal and internal adapter occurrences, providing a unified solution for adapter trimming across diverse direct RNA sequencing datasets.}
...
\begin{table}[htbp]
    \centering
    \caption{DeepChopper Runtime and Resource Usage for VCaP (9,177,639 Reads)}
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Pipeline Stage} & \textbf{Trial 1} & \textbf{Trial 2} & \textbf{Trial 3} & \textbf{Average} \\
        \midrule
        \multicolumn{5}{l}{\textit{Runtime}} \\
        FASTQ to Parquet Conversion & 24 min & 25 min & 26 min & 25 min \\
        Adapter Prediction & 3 h 45 min & 3 h 41 min & 3 h 40 min & 3 h 42 min \\
        Post-processing & 51 min & 58 min & 56 min & 55 min \\
        \textbf{Total} & \textbf{5 h} & \textbf{5 h 4 min} & \textbf{5 h 2 min} & \textbf{5 h 2 min} \\
        \midrule
        \multicolumn{5}{l}{\textit{Peak Memory Usage}} \\
        CPU Memory (FASTQ Conversion) & 39 GB & 40 GB & 41 GB & 40 GB \\
        CPU Memory (Prediction) & 3.5 GB & 3.7 GB & 3.9 GB & 3.7 GB \\
        CPU Memory (Post-processing) & 40 GB & 41 GB & 42 GB & 41 GB \\
        GPU Memory (Prediction) & 32 GB & 33 GB & 34 GB & 33 GB \\
        \bottomrule
    \end{tabular}
    \label{tab:st2}
%     \footnotetext{All benchmarks conducted on high-performance computing infrastructure with 16 CPU cores, 60 GB RAM, and dual NVIDIA A100 GPUs (80 GB memory each). Adapter prediction stage used a batch size of 64.}
\end{table}
...
\begin{figure}[!ht]
	...
	\caption{{\bf Performance evaluation in a held-out test dataset showing Recall, Precision, and F1 values for DeepChopper.}}\label{fig:sf1}
\end{figure}
