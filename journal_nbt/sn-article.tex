...
<<<<<<< HEAD
DeepChopper incorporates \gls{lcglm} HyenaDNA~\cite{nguyen2024hyenadna}, which excels at modeling long-range dependencies~\figref{fig:f1}{a}.
While HyenaDNA cannot directly process sequence base quality information, DeepChopper addresses this limitation through a dedicated quality block, a neural network comprising multiple \glspl{mlp} with residual connections~\cite{he2015deep}, to effectively utilize this crucial feature~\figref{fig:f1}{b} (See \hyperref[sec:methods]{Methods} for details).
This hybrid architecture combines broad contextual understanding with nucleotide-level precision~\cite{poli2023hyena}, enabling DeepChopper to effectively identify and process internal adapters~\figref{fig:f1}{a}.
The system splits reads containing internal adapters into multiple records while simultaneously trimming $3'$ end adapters, thus preserving authentic biological sequences and eliminating chimeric artifacts.
||||||| parent of 9549e4e (update figure 1)
DeepChopper integrates \gls{lcglm} HyenaDNA~\cite{nguyen2024hyenadna}, optimized for efficient long-range dependency modeling~\figref{fig:f1}{a}.
However, HyenaDNA cannot take the base quality of the sequence as input.
Hence, DeepChopper uses a nuerual network named quality block that includes multiple \glspl{mlp} and residual connection~\cite{he2015deep} in order to use the base quality of sequence efficiently~\figref{fig:f1}{b} since the base quality of the sequence is an important feature~\figref{fig:f1}{b} (See \hyperref[sec:methods]{Methods} for details).
This hybrid architecture enables both broad context understanding and precise single-nucleotide resolution~\cite{poli2023hyena} (\figref{fig:f1}{a}).
Through this comprehensive approach, DeepChopper effectively splits reads containing internal adapters into multiple records while trimming adapters at the $3'$ ends, thereby preserving authentic biological sequences and eliminating chimera artifacts (\figref{fig:f1}{a}).
=======
DeepChopper integrates \gls{lcglm} HyenaDNA~\cite{nguyen2024hyenadna}, optimized for efficient long-range dependency modeling~\figref{fig:f1}{a}.
However, HyenaDNA cannot take the base quality of the sequence as input.
Hence, DeepChopper uses a nuerual network named quality block that includes multiple \glspl{mlp} and residual connection~\cite{he2015deep} in order to use the base quality of sequence efficiently~\figref{fig:f1}{b} since the base quality of the sequence is an important feature~\figref{fig:f1}{b} (See \hyperref[sec:methods]{Methods} for details).
This hybrid architecture enables both broad context understanding and precise single-nucleotide resolution~\cite{poli2023hyena} (\figref{fig:f1}{a}).
Through this comprehensive approach, DeepChopper effectively splits reads containing internal adapters into multiple records while trimming adapters at the $3'$ ends, thereby preserving authentic biological sequences and eliminating chimera artifacts (\figref{fig:f1}{a}).
>>>>>>> 9549e4e (update figure 1)

To enhance prediction accuracy, DeepChopper implements a post-processing stage using a sliding window and majority vote approach~\figref{fig:f1}{c}.
The system applies a 21-nucleotide sliding window with stride 1 across the read, analyzing the distribution of predicted adapter positions within each window (See \hyperref[sec:methods]{Methods} for details).
This refinement process ensures predicted adapter sequences are contiguous and biologically plausible by smoothing irregularities and eliminating isolated false positives, while preserving genuine adapter signals that demonstrate consistent patterns across adjacent windows.

Compared to existing general-purpose \glspl{glm} like DNABERT2~\cite{zhou2023dnabert2} and Nucleotide Transformer~\cite{dalla2023nucleotide}, DeepChopper is specifically designed for long-read sequence analysis.
It processes up to 32 kilobase pairs, a sufficiently long context to capture most mRNAs, and employs single-nucleotide tokenization to identify non-human reference sequences, such as \gls{ont} adapters, with base-pair precision.
...
% Result
\begin{figure}[!ht]
	\includegraphics[height=0.51\columnwidth]{finals/figure2}
...
A similar pattern of improper inter-chromosomal connections was also observed in VCaP RNA004 \gls{drs} data (\edfigref{fig:sf3}{c}), suggesting that chimera artifacts may be an inherent issue of \gls{drs} technology, rather than being fully resolved by improvements in sequencing chemistry.
...
Similar results were also noted for VCaP RNA004 \gls{drs} data (\edfigref{fig:sf3}{d}).
...
Upon closer examination of the chimera artifact-derived gene fusion calls, we found a notable enrichment of ribosomal protein genes (\edfigref{fig:sf4}{a}).
\gls{go} enrichment analysis in VCaP (\figref{fig:f2}{d}) and \gls{sgnex} cell lines (\edfigref{fig:sf4}{b}) confirmed that housekeeping ribosomal protein genes are frequently involved in these artifacts.
Additionally, ribosomal protein genes were also enriched in the chimera artifacts associated with RNA004 chemistry (\edfigref{fig:sf4}{b}).
...
At its core, DeepChopper employs HyenaDNA~\cite{nguyen2024hyenadna} as its primary feature extractor, processing nucleotide sequences into rich 256-dimensional feature representations.
These features are then refined through a quality block that incorporates standardized base quality scores.
The quality block, comprising two \glspl{mlp} with residual connections, processes normalized quality information to prevent numerical instability while enhancing the model's sequence interpretation capabilities.
The processed features are subsequently fed into a classification head that computes per-nucleotide adapter probabilities.
Through this binary classification approach, DeepChopper generates precise adapter/non-adapter predictions for each nucleotide position across the input sequence, enabling comprehensive adapter identification.
...
\begin{figure}[!ht]
    \includegraphics[height=0.18\columnwidth]{finals/sf2}
