...
Most of the reads contained 1-5 intervals, with multiple intervals that could indicate fragmented chimeric sequences often missed by Dorado~\cite{dorado2023} (\figref{fig:f1}{d}).
...
\begin{figure}[!h]
	...
	\caption{{\bf Characterization of \gls{fp} chimeric reads Detected by DeepChopper in the VCaP 002 dataset.} (a) Comparison of gene expression levels between reference genes and \gls{fp} genes in the VCaP 002 dataset. \gls{fp} genes show significantly higher expression levels (\(\textrm{p-value} < 2.2 \times 10^{-16}\)). (b) Distribution of gene effective sizes for all genes and target genes, showing similar lengths between reference and \gls{fp} genes. (c) Overlap of \gls{go} pathways enriched among \gls{fp} genes across six cell lines (VCaP, A549, K562, HepG2, MCF7, HCT116). The figure shows intersection sizes of enriched \gls{go} terms, highlighting common pathways related to ribosomal biogenesis and translation. (d) Gene fusion detection in VCaP 002 dataset. Comparison of DeepChopper and Dorado (with trimming) performance. The blue bars represent gene fusions unsupported by \gls{ont} \gls{pcr} cDNA, while the green bars indicate gene fusions supported by \gls{ont} \gls{pcr} cDNA. (e) Example of a misleading gene fusion caused by an artificial chimeric read containing internal synthetic sequences in the VCaP 002 dataset. Quality score (green line), electrical signal (blue line), and open pore signal (red circle) are shown. PolyA region (red rectangle) and chopped sequences (grey dashed lines) illustrate DeepChopper's segmentation of synthetic sequences.}\label{fig:f2}
\end{figure}

% Comparison of Gene Expression Levels Between Reference and FP Genes (Fig 2a) Fig 2b
...
This result suggests that \gls{fp} genes are characterized by higher expression levels, which may indicate their association with artificial sequences or chimeric reads rather than true biological signals.\yy{add more result description for figure 2b; refer to tingyou}

% Visualization of Chromosomal Connections in FP Chimeric Reads (Supplementary Fig 2)
...
DeepChopper significantly outperformed Dorado, reducing total gene fusion events by 89\% and increasing the proportion of supported fusions (validated by ONT PCR cDNA) from 2\% (Dorado) to 17\%  (\figref{fig:f2}{d}). 
...
This precision is crucial for applications in cancer genomics and transcriptomics, where correct identification of gene fusions is critical \yy{may need to add citations}.
...
This cross-platform validation assessed the ability of DeepChopper to maintain high support rates across various sequencing conditions.
...
Following the identification of the \gls{fp} group, we proceeded to assess gene expression levels specifically associated with these reads using IsoQuant (v )~\cite{prjibelski2023accurate}. \yy{add versions; refer to tingyou}



\subsection{\gls{go} analysis}

\yy{please help add more details about parameters or versions if posssible; refer to tingyou}
...
\subsection{Gene fusion analysis}

\yy{please help add methods for figure 2d and 2e; refer to tingyou. IF need citations just copy the paper title in the manuscript.}
...
\begin{figure}[!h]
	...
	\caption{ {\bf  Visualization of chromosomal connections for \gls{fp} chimeric reads detected in the VCaP 002 dataset.} Each arc represents a connection between two chromosomal regions. The length of each chromosome segment is proportional to the number of connections involving that chromosome.}
	\label{fig:sf2}
\end{figure}
