...
Given DeepChopper’s ability to increase the yield of \gls{drs} reads by correcting chimera artifacts, we hypothesized that it could enhance downstream transcript detection and quantification. Using IsoQuant~\cite{prjibelski2023accurate} for transcript annotation from VCaP \gls{drs} data, we found that DeepChopper nearly doubled the number of identified transcripts compared to Dorado adapter-trimmed long reads (\figref{fig:f2}{b}). This improvement was particularly evident in the increased detection of full-length transcripts, as indicated by the rise in the \gls{fsm} category (\figref{fig:f2}{b}). Additionally, DeepChopper improved the discovery of alternatively spliced transcripts, including those classified as \gls{ism}, \gls{nic}, and \gls{nnc}, further demonstrating its effectiveness in improving transcript diversity and quantification accuracy.

Finally, we evaluated whether chimera artifact correction by DeepChopper could improve gene fusion detection. We applied FusionSeeker for gene fusion detection from VCaP \gls{drs} and direct cDNA sequencing long-reads. We observed that 
